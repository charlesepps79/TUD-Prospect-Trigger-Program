...
*** CREATE WATERFALL OF EXCLUSION REASONS... --------------------- ***;
	...
	IF CUSTOMERTYPE ='PB' THEN REASON = '1. PB APP'; 
	ELSE 
	IF LOANNUMBER NE '' THEN REASON = '2. BOOKED LOAN'; 
   *ELSE
	...
	ELSE 
	if taskstatus = 'DENIED' THEN REASON = '4. TASK STATUS=DENIED'; 
	ELSE 
	IF riskrank_small = 'FAIL' THEN REASON='5. RISK RANK FAIL'; 
   *ELSE 
   ...
   *ELSE 
	...
	If  SUBSTR(REASON, 1, 2) IN ('8.' '99') AND 
		CUSTOMERELIG NE 'ELIGIBLE' THEN DO;
	...
	END;
*** Custom Score >= 240 & FICO >= 600 ---------------------------- ***;
*** Custom Score >= 270 and FICO >= 575 -------------------------- ***;
	IF reasondeclined = '' 
...
*** CLEAN UP SOME BAD STATE FORMATS ------------------------------ ***;
	IF STATE IN ('AL' 'OK' 'NM' 'NC' 'GA' 'TN' 'MO' 'WI' 'SC' 'TX' 'VA' 
		...
		THEN STATE=STATE;  
	ELSE DO;
  		IF STATE IN ('AL.' 'ALA') THEN STATE='AL'; 
		ELSE 
  		IF STATE = 'AZ' THEN STATE = 'NM'; 
		ELSE 
  		IF STATE = 'FL' THEN STATE = 'GA'; 
		ELSE 
  		IF SUBSTR(STATE, 1, 1) = 'G' THEN STATE = 'GA'; 
		ELSE 
  		IF STATE = 'KS' THEN STATE = 'MO'; 
		ELSE 
  		IF STATE = 'TEX' THEN STATE = 'TX'; 
		ELSE 
  		IF STATE = 'TX.' THEN STATE = 'TX'; 
		ELSE 
  		IF STATE = 'WI.' THEN STATE = 'WI';
  		ELSE STATE = '??';
	END;
...
DATA KEEPERS;
	...
	SET APPS;
	WHERE REASON = '99. KEEP THE APPLICATION';
   *WHERE REASON = '9. NOT BRANCH APPROVED';
...
*** Stephen wants to see all 8396 of these, not just those who     ***;
*** would survive the rest of the exclusions --------------------- ***;
DATA INELIG;
	format task_refno ApplicationEnterDateOnly taskstatus housingstatus 
		   customertype reasondeclined creditscore custom_score 
		   riskrank_small riskrank_large freeincome monthlygrossincome 
